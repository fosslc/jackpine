...
public class SpatialSqlDialectForMysql extends SqlDialectForMysql implements SpatialSqlDialect
{
  ...
  public String getSelectAllFeaturesWithinADistanceFromPoint()
  {
	  StringBuffer sb = new StringBuffer();
	    ...
	    //sb.append(table.getName()).append("_merge");
  ...
  public String getSelectLongestLine(){
	  ...
	  // Generate the join SQL statement.
	  sb.append("SELECT  fullname, Length(shape) AS line FROM edges_merge order BY line DESC limit 1");
	  String sql = sb.toString();
	  // System.out.println("The query "+ sql);
	  return sql;
...
//all the polygons that are intersected by the longest line: 1
  public String[] getSelectLongestLineIntersectsArea(){ 
	  ...
	  queries[1] = "select count(*) from  areawater_merge a, edges_merge e where Intersects(e.shape, a.shape) and e.ogr_fid= ?";
  ...
  public String[] getSelectAreaOverlapsLargestArea(){ 
	  ...
	  String[] queries =  new String[2]; 
	  queries[0] = "select ogr_fid from arealm_merge order by Area(shape) desc limit 1";
	  queries[1] = "select count(*) from  arealm_merge al, areawater_merge aw where Overlaps(al.shape, aw.shape) and al.ogr_fid= ?";
  ...
  public String[] getSelectLargestAreaContainsPoint(){ 
	  ...
	  queries[1] = "select count(*) from  pointlm_merge pl, areawater_merge aw where Contains(aw.shape, pl.shape) and aw.ogr_fid= ?";
  ...
  public String getSelectLineOverlapsLine(){ 
	  ...
	  // Generate the join SQL statement. 
	  sb.append("select e1.fullname from  edges_merge e1 , edges_merge e2 where overlaps(e1.shape, e2.shape) limit 1 ");
	  String sql = sb.toString();
	  // System.out.println("The query "+ sql);
	  return sql;
  }
  ...
  public String getSelectLineCrossesLine(){ 
	  ...
	  // Generate the join SQL statement. 
	  sb.append("select e1.fullname from  edges_merge e1 , edges_merge e2 where crosses(e1.shape, e2.shape) limit 1 ");
	  String sql = sb.toString();
	  // System.out.println("The query "+ sql);
	  return sql;
  }
  ...
  public String getInsertIntoEdgesMerge(){
	  ...
	  sb.append("insert into edges_merge values (0, GeomFromText('LINESTRING(").append(nxt1)
  ...
  public String[] getLandUseQueries() { 
	...
	queries[3] = "select lake.wtr_nm, geo_id, pa.marketvalu as property_value from parcels2008 as pa, s_wtr_ar as lake where  (GLength(LineStringFromWKB(LineString( Centroid(lake.shape), Centroid(pa.shape) )))) <= 100 and pa.marketvalu> 0 order by property_value desc limit 10"; 
				  
	
	//Which office buildings have front yard parking restrictions 
	queries[4] = "select property_i from  land_use_2006 as lu, frontyard_parking_restrictions fypr where lu.general_la=400  and  Overlaps(fypr.shape, lu.shape) ";
				  
	
	// Show the commercial properties that are built on un-permitted landfills
	queries[5] = "select lu.property_i from  land_use_2006 as lu, landfills lf where lu.general_la=300 and lf.permitted='UNPERMITTED' and Intersects(lf.shape,lu.shape) ";
  ...
  public String[] getEnvHazardQueries() { 
		...
		//queries[2] = "sselect   sum(area(fa.shape)/43560) as hr_flood_area from s_fld_haz_ar fa, s_wtr_ar wa where wa.wtr_nm='Lake Travis' and (fld_zone='A' or fld_zone='AE' or fld_zone='AO' or fld_zone='V') and (GLength(LineStringFromWKB(LineString( Centroid(fa.shape), Centroid(wa.shape) )))) < 5280 limit 1 ";
